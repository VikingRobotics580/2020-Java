...
public class TutorialSubsystem extends Subsystem { //Change 'TutorialSubsystem' to the name of the class.
    ...
    public void tutorialCode() {
        ...
        if(controller.getRawButton(1)) //Gets input from the joystick button 'TUT_Val'. If pressed, the next line runs.
            System.out.println(value); //Prints 'value' to the terminal

        if(controller.getRawButton(1)) //Gets input from the joystick button 'TUT_Neg'. If pressed, the next line runs.
            value += 1; //Increases the value of 'value' by one every time the method runs and the button is pushed.
        else if(controller.getRawButton(1)) //Gets input from the joystick button 'TUT_Neg'. If pressed, the next line runs.
